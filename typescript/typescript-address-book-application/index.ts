...
function addContact(contact: Contact): void {
  ...
  if (addressBook.some((entry) => entry.id === contact.id)) {
    ...
    console.log(
      `Contact id is already existing. Assigning new id:${idCounter}`
    );
...
function searchContacts(query: string): Contact[] {
  ...
  return addressBook.filter((contact) => {
    return (
      contact.name.trim().toLowerCase().includes(queryNormalized) ||
...
console.log(
  "address book after deletion of first entry with id 1:",
  addressBook
);
